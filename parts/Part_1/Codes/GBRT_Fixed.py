...
def run_gbrt():
    print("Running GBRT (Fixed Split - Single Pass Data Collection)...")
    ...
    for t in targets:
        ...
        metrics = evaluate_portfolio(test_df, model.predict(X_test), t, f"GBRT Fixed_{t}", results_base)

        metrics = evaluate_portfolio(test_df, model.predict(X_test), t, f"GBRT Fixed_{t}", results_base)
        
        with open(os.path.join(results_base, 'datas', f'GBRT_{t}_report.txt'), 'w') as f_out:
