...
def run_gbrt_optimized():
    print("Running GBRT (Optimized Fixed - Single Pass)...")
    ...
    # Load Best Params
    best_params = {t: {
        'n_estimators': int(get_best_param(csv_path, 'GBRT', t, 'n_estimators', 50)),
        'max_depth': int(get_best_param(csv_path, 'GBRT', t, 'max_depth', 2))
    } for t in targets}
    ...
    for t in targets:
        ...
        if not data[t]['train_X']: continue
        ...
        model = GradientBoostingRegressor(n_estimators=best_params[t]['n_estimators'], max_depth=best_params[t]['max_depth'], random_state=42)
        model.fit(X_train, y_train)
        metrics = evaluate_portfolio(test_df, model.predict(X_test), t, f"GBRT_Opt_{t}", results_base)
        with open(os.path.join(results_base, 'datas', f'GBRT_{t}_optimized_fixed_report.txt'), 'w') as f:
            f.write(f"GBRT Optimized Results ({t})\n" + "="*40 + "\n" + format_metrics(metrics))
