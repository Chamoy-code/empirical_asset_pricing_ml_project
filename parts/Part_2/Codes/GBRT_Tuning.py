...
def run_gbrt_tuning():
    print("Running GBRT Tuning (3-Way Split - Single Pass)...")
    ...
    for f in files:
        ...
        for t in targets:
            ...
            if year < TRAIN_END:
                ...
                data[t]['val_X'].append(X); data[t]['val_y'].append(valid_chunk[t].values.astype(np.float32))
            else:
    ...
    for t in targets:
        ...
        for p in params_grid:
            model = GradientBoostingRegressor(n_estimators=p['n_estimators'], max_depth=p['max_depth'], random_state=42)
            model.fit(X_train, y_train)
            mse = np.mean((y_val - model.predict(X_val))**2)
            if mse < best_mse: best_mse, best_p, winner = mse, p, model
        
        print(f"    Target {t} -> Best Params: {best_p}")
        pd.DataFrame([
            {'Model': 'GBRT', 'Target': t, 'Best_Parameter': 'n_estimators', 'Best_Value': best_p['n_estimators']},
            {'Model': 'GBRT', 'Target': t, 'Best_Parameter': 'max_depth', 'Best_Value': best_p['max_depth']}
        ]).to_csv(csv_path, mode='a', header=not os.path.exists(csv_path), index=False)
        
        metrics = evaluate_portfolio(test_df, winner.predict(X_test), t, f"GBRT 3-Way {best_p['n_estimators']}t {best_p['max_depth']}d_{t}", results_base)
        with open(os.path.join(results_base, 'datas', f'GBRT_{t}_tuning_report.txt'), 'w') as f_out:
            f_out.write(f"GBRT Results ({t})\nWinner: {best_p}\n" + "="*30 + "\n" + format_metrics(metrics))
